...
function Bookings_by_Passenger_Type() {
        ...
        if (data.status) {
          ...
          setTotal(
            parseInt(data.data[0].bookings) +
              parseInt(data.data[1].bookings) +
              parseInt(data.data[2].bookings)
          );
        }
      });
  }, []);
  ...
  return (
    ...
    display : "flex",}}>
            <h1>Booking By Each Passenger Type
            </h1>
    ...
    >
      ...
      <div className="table-responsive">
        ...
        <label>
          Platinum:{" "}
          {Math.round((platinum_amount / total) * 100).toString() + "%"}
        </label>
        <div class="progress">
          <div
            ...
            role="progressbar"
            style={{
              width: ((platinum_amount / total) * 100).toString() + "%",
            }}
            aria-valuenow={(platinum_amount / total).toString()}
            aria-valuemin="0"
            aria-valuemax="100"
          ></div>
        </div>
        ...
        <label>
          Bussiness:{" "}
          {Math.round((bussiness_amount / total) * 100).toString() + "%"}
        </label>
        <div class="progress">
          <div
            ...
            role="progressbar"
            style={{
              width: ((bussiness_amount / total) * 100).toString() + "%",
            }}
            aria-valuenow={(bussiness_amount / total).toString()}
            aria-valuemin="0"
            aria-valuemax="100"
          ></div>
        </div>
        <br></br>
        <label>
          Economy: {Math.round((economy_amount / total) * 100).toString() + "%"}
        </label>
        <div class="progress">
          <div
            className="progress-bar bg-warning"
            role="progressbar"
            style={{ width: ((economy_amount / total) * 100).toString() + "%" }}
            aria-valuenow={(economy_amount / total).toString()}
            aria-valuemin="0"
            aria-valuemax="100"
          ></div>
        </div>
