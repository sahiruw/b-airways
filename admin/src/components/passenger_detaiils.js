...
function PassengerDetails(){
    ...
    const get_details = (e) => {
        ...
        .then((data) => {
            ...
            for(let i = 0; i < data.flightdataMembers.length; i++){
                ...
                if(age >= 18){
                    setMemberDetailsAbove18([...member_details_above18,data.flightdataMembers[i]]);
                    console.log(data.flightdataMembers[i].fname,"above 18 member")
                
                }
                if(age < 18){
                    console.log(data.flightdataMembers[i].fname,"below 18 member")
                    setMemberDetailsBelow18([...member_details_below18,data.flightdataMembers[i]]);
            ...
            for(let i = 0; i < data.flightdataGuest.length; i++){
                ...
                if(age >= 18){
                    ...
                    setGuestDetailsAbove18([...guest_details_above18,data.flightdataGuest[i]]);
                    
                }
                if(age < 18){
                    console.log(data.flightdataMembers[i].fname,"below 18 guest")
                    setGuestDetailsBelow18([...guest_details_below18,data.flightdataGuest[i]]);
    ...
    return(
        <div>
            ...
            <input type="number"  min="1" max="9" style= {{marginLeft:100}} onChange={(e) => {
